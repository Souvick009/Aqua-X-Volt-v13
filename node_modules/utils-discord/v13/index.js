...
module.exports = class DiscordUtility {
    ...
    static async awaitPlayers(msg, join, emoji, max, min = 1) {
        ...
        await send(msg, {
            content: `You will need at least ${min - 1} more player(s) (at max ${max - 1}). To join, type \`${join}\`.`
        },
            false
        );
        const joined = [];
        ...
        const verify = await msg.channel.awaitMessages({
            filter,
            max: max - 1,
            time: 60000
        });
    ...
    static async createEmbedPages(client, message, array, options = {
        emojis: {}
    },  isUserlogs) {
        ...
        if (!isUserlogs) {
            ...
            if (type === "mutes") {
                header = `**Total Mutes Recived: ${array.mutes.length}** \n --------------------------------------------`
                newArray = array.mutes
            } else {
                newArray = array.timeouts
                header = `**Total Timeouts Recived: ${array.timeouts.length}** \n --------------------------------------------`

            }
        }
        let pageno = parseInt(args) ? args : 1;
        parseInt(args) <= Math.ceil(newArray.length / perpage) ? pageno : pageno = 1;
        ...
        embed.setColor(color)
        embed.setDescription(`${header ? `${header}` : ""} \n ${newArray.slice(first, second).join(joinBy)}`);
        if (options.watermark && watermark) embed.setDescription(`${embed.description}\n\n${watermark ? `\n\n${watermark}` : ""}`);
        if (title) embed.setTitle(title)
        if (thumbnail) embed.setThumbnail(thumbnail)
        embed.setFooter(`${footer} | Page: ${pageno}/${Math.ceil(newArray.length / perpage)}`, footerImage)
        if (timestamp) embed.setTimestamp();
        if (author) embed.setAuthor({
            name: author,
            iconURL: authorImage
        ...
        let ending = new Discord.MessageButton()
            ...
            .setLabel(buttonLast)
            .setStyle("PRIMARY");
        var deleteEmbed = new Discord.MessageButton()
        ...
        if (isUserlogs && array.timeouts.length !== 0) {
            mutesortimeouts = new Discord.MessageButton()
                .setCustomId("change")
                .setLabel("Show Timeouts")
                .setStyle("PRIMARY");
            let row1 = new Discord.MessageActionRow().addComponents([starting, back, next, ending, deleteEmbed])
            let row = new Discord.MessageActionRow().addComponents([mutesortimeouts])
            comps = [row1, row]
        } else {
            comps = [new Discord.MessageActionRow().addComponents([starting, back, next, ending, deleteEmbed])]

        }
        let msg;

        if (newArray.length < perpage && isUserlogs) {
            if(array.timeouts.length !== 0){
            mutesortimeouts = new Discord.MessageButton()
                .setCustomId("change")
                .setLabel("Show Timeouts")
                .setStyle("PRIMARY");
            comps = [new Discord.MessageActionRow().addComponents([mutesortimeouts])]
            }
            msg = await send(message, {
                components: comps,
                embeds: [embed]
            }, false);
        }
        ...
        if (newArray.length < perpage || !buttons) {
            if (comps.length === 0) {
                msg = await send(message, {
                    embeds: [embed]
                }, false);
            }
        } else if (newArray.length > perpage && buttons) {
            msg = await send(message, {
                components: comps,
                embeds: [embed]
            }, false);
        ...
        if (newArray.length > perpage || comps.length !== 0) {
            if (!buttons) {
                ...
                collector.on('collect', async (r) => {
                    ...
                    if (r.emoji.name === forward && reactionadd !== 0) {
                        ...
                        first += perpage;
                        second += perpage;
                        embed.setDescription(`${header ? `${header}` : ""} \n ${array.slice(first, second).join(joinBy)}${watermark ? `\n\n${watermark}` : ""}`);
                ...
                //     time: 120000
                // });
                const collector = new Discord.InteractionCollector(client, {message: msg, time: 120000, filter: (button) => button.user.id === message.member.user.id, componentType: "BUTTON"});
                collector.on("collect", async (button) => {
                    await button.deferUpdate();
                    const reactionadd = newArray.slice(first + perpage, second + perpage).length;
                    const reactionremove = newArray.slice(first - perpage, second - perpage).length;
                    if (button.customId === "starting") {
                        pageno = 1;
                        first = 0;
                        second = perpage;
                        embed.setDescription(`${header ? `${header} \n ` : ""} ${newArray.slice(first, second).join(joinBy)}${watermark ? `\n\n${watermark}` : ""}`);
                        embed.setFooter(`${footer} | Page: ${pageno}/${Math.ceil(newArray.length / perpage)}`, footerImage);
                        msg.edit({
                            components: comps,
                            embeds: [embed]
                        ...
                        pageno = Math.ceil(newArray.length / perpage);
                        first = (pageno * perpage) - perpage;
                        second = pageno * perpage;
                        embed.setDescription(`${header ? `${header} \n ` : ""}${newArray.slice(first, second).join(joinBy)}${watermark ? `\n\n${watermark}` : ""}`);
                        embed.setFooter(`${footer} | Page: ${pageno}/${Math.ceil(newArray.length / perpage)}`, footerImage);
                        ...
                        await msg.delete();
                    } else if (button.customId === "change" && isUserlogs) {
                        if (type === "mutes") {
                            type = "timeouts"
                            newArray = array.timeouts
                            mutesortimeouts.setLabel("Show Mutes")
                            header = `**Total Timeouts Recived: ${array.timeouts.length}** \n --------------------------------------------`

                        } else {
                            type = "mutes"
                            newArray = array.mutes
                            mutesortimeouts.setLabel("Show Timeouts")
                            header = `**Total Mutes Recived: ${array.mutes.length}** \n --------------------------------------------`
    ...
    static async mongoUpdate(schema, options, value) {
        try {
            await schema.updateOne(options, {
                "$set": value
            }).catch(e => {
                throw new TypeError("Something went wrong:", e)
            })
